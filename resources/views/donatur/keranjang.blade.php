...
<form id="delete-form-{{ $item->id }}" action="{{ route('donatur.keranjang.destroy', $item->id) }}" method="POST" class="hidden">
    @csrf
    @method('DELETE')
</form>
@endforeach
...
<div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8 font-sans" style="background-color: var(--color-putih);">
    <form id="checkoutForm" action="{{ route('donatur.orders.checkout') }}" method="POST" class="max-w-7xl mx-auto">
        @csrf
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('donatur.keranjang.create') }}" class="px-5 py-2.5 rounded-xl bg-white font-medium shadow-sm transition-all duration-200 flex items-center gap-2 hover:-translate-y-1" style="border: 1px solid var(--color-kuning); color: var(--color-coklat);">
        ...
        <div class="card-admin p-16 text-center max-w-2xl mx-auto mt-10">
            ...
            <a href="{{ route('donatur.keranjang.create') }}" class="btn-primary inline-flex items-center px-8 py-3.5">
                Mulai Belanja
            </a>
        </div>
        @endif
...
<script>
    ...
    function updateQty(id, action, manualQty = null) {
        ...
        fetch("/donatur/keranjang/" + id + "/ajax", {
            method: "PUT",
            headers: {
    ...
    if (deleteBtn) {
        deleteBtn.onclick = function() {
            ...
            if(confirm("Yakin ingin menghapus barang yang dipilih?")) {
                fetch("/donatur/keranjang/delete-selected", {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": token
